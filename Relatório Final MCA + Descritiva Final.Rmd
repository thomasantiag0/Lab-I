...
ggplot(
  plotdata,
  aes(
    x = reorder(geographic_region_br, -pct),
    y = pct
  )
) +
  geom_bar(
...
ggplot(
  plotdata,
  aes(
    x = reorder(races_categorized, -pct),
    y = pct
  )
) +
  geom_bar(
...
ggplot(
  plotdata,
  aes(
    x = reorder(highest_degree_ALL_br_y, -pct),
    y = pct
  )
) +
  geom_bar(
...
tab1(job_before, decimal = 2, cum.percent = TRUE, graph = F, main = "Emprego antes da pandemia", xlab = "Tipo de emprego", ylab = "Número de observações")
```
...
tab1(job_after, decimal = 2, cum.percent = TRUE, graph = F, main = "Emprego na pandemia", xlab = "Tipo de emprego", ylab = "Número de observações")
```
...
Quantidade_de_cuidadores <- db %>%
  dplyr::select(bg_caregivers_b4_br) %>%
  drop_na()

tab1(Quantidade_de_cuidadores, decimal = 2, cum.percent = TRUE, graph = F, main = "Quantidade de cuidadores por residência", xlab = "Número de cuidadores", ylab = "Número de residências")
```
...
Quantidade_de_criancas <- db %>%
  dplyr::select(bg_num_children_br) %>%
  drop_na()

tab1(Quantidade_de_criancas, decimal = 2, sort.group = "decreasing", cum.percent = TRUE, graph = F, main = "Quantidade de crianças por residência", xlab = "Número de crianças", ylab = "Número de residências")
```
...
tab1(restrictions, decimal = 2, cum.percent = TRUE, graph = F, main = "Tipo de quarentena", xlab = "Tipo de quarentena", ylab = "Número de observações")
```
...
tab1(stress, decimal = 2, cum.percent = TRUE, graph = F, main = "Nível de estresse com o isolamento", xlab = "Nível", ylab = "Número de observações")
```
...
plot(db_bloco_1_pref[, 1],
  main = "Alguém lia p/ criança",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_1_pref[, 2],
  main = "Pais liam p/ criança",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_1_pref[, 3],
  main = "Pais liam c/ criança",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_1_pref[, 4],
  main = "Criança lia sozinha",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_1_pref[, 5],
  main = "Tempo de leitura com a criança",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
...
```{r}
# Ajuste do MCA
res.mca_pref_1 <- MCA(db_bloco_1_pref, ncp = 5, graph = F)
```

No gráfico biplot abaixo cada alternativa para cada resposta do questionário é plotada em vermelho, enquanto que cada indivíduo (pessoa que respondeu o questionário) é plotado em vermelho. Estas obervações são plotadas em relação as duas dimensões mais explicativas, no caso as dimensões 1 e 2 e indica o quão bem cada dimensão explica aquele observação.
Pode-se observar que há uma quantidade grande de indivíduos no canto inferior esquerdo do gráfico que não são bem explicados por pelas duas dimensões.



```{r, fig.cap = "Figura n - Representação do quanto as duas principais dimensões explicam cada dividuo e variavel"}
fviz_mca_biplot(res.mca_pref_1,
...
```{r, fig.cap = "Figura n - Representação gráfica da porcentagem de variância explicada por cada dimensão"}
# Plot dos 10 maiores autovalores
fviz_screeplot(res.mca_pref_1, addlabels = TRUE, ylim = c(0, 10))
```

> Dentre das 15 principais categorias do gráfico anterior, o plot de quanto elas contribuem para as duas maiores dimensões

```{r, fig.cap = "Figura n - Representação do quanto cada altenativa do questionário contribui para as pricipais"}
par(mfrow = c(2, 1))
# Contributions of rows to dimension 1
fviz_contrib(res.mca_pref_1, choice = "var", axes = 1, top = 15)
# Contributions of rows to dimension 2
fviz_contrib(res.mca_pref_1, choice = "var", axes = 2, top = 15)
```
...
```{r}
res.mca_posf_1 <- MCA(db_bloco_1_posf, ncp = 5, graph = F)
```
...
```{r, fig.cap = "Figura n - Representação do quanto as duas principais dimensões explicam cada dividuo e variavel"}
fviz_mca_biplot(res.mca_posf_1,
  repel = TRUE, # Avoid text overlapping (slow if many point)
  ggtheme = theme_minimal()
)
```
...
```{r, fig.cap = "Figura n - Representação gráfica da porcentagem de variância explicada por cada dimensão"}
# Plot dos 10 maiores autovalores
fviz_screeplot(res.mca_posf_1, addlabels = TRUE, ylim = c(0, 10))
```
...
par(mfrow = c(2, 1))
# Contributions of rows to dimension 1
fviz_contrib(res.mca_posf_1, choice = "var", axes = 1, top = 15)
# Contributions of rows to dimension 2
fviz_contrib(res.mca_posf_1, choice = "var", axes = 2, top = 15)
```
...
| Pré Covid | Com Covid |
| --------- | --------- |
| hlr_books_b4_corrected | hlr_books_since_corrected |
| hlr_dbooks_b4_corrected | hlr_dbooks_since_corrected |
| hlr_games_b4_corrected | hlr_games_since_corrected |
| hlr_dgames_b4_corrected | hlr_dgames_since_corrected |
| hlr_ad_books_b4_corrected | hlr_ad_books_since_corrected |
| hlr_ad_dbooks_b4_corrected | hlr_ad_dbooks_since_corrected |
| hlr_ad_news_b4_corrected | hlr_ad_news_since_corrected |
...
Tanto para o bloco pré-pandemia como para o pós-pandemia, pode-se observar que as variávies **books** | **ad_books** e **games** | **dgames** possuem a maior correlação, indicando que, casas com uma maior quantidade de livros para crianças tendem a ter também uma maior quantidade de livros para adultos. O mesmo acontece entre jogos e jogos digitais.

Há correlações que aumentaram e outras que diminuiram no período pandemico, no entanto, a maior diferença é de 0.08. 
...
t_pos <- "Pós Pandemia"
par(mfrow = c(1, 2))
corrplot(matcorr_pre_2, method = "circle", type = "upper", diag = FALSE, title = t_pre, addCoef.col = "black", mar = c(0, 0, 2, 0), number.cex = 0.8)
corrplot(matcorr_pos_2, method = "circle", type = "upper", diag = FALSE, title = t_pos, addCoef.col = "black", mar = c(0, 0, 2, 0), number.cex = 0.8)
```
<br />
...
plot(db_bloco_2_pref[, 1],
  main = "Qtd Livros impressos para crianças",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_pref[, 2],
  main = "Qtd Livros digitais para crianças (e-
books no Kindle)",
  ylab = "Count",
  col = "steelblue",
  las = 2
...
plot(db_bloco_2_pref[, 3],
  main = "Qtd Jogos educativos de tabuleiro/cartas",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_pref[, 4],
  main = "Qtd Jogos educativos digitais",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_pref[, 5],
  main = "Qtd Livros impressos para adultos",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_pref[, 6],
  main = "Qtd Livros digitais ou assinaturas de jornais para
adultos",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_pref[, 7],
  main = "Assinaturas de jornais para adultos",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
...
```{r}
# Ajuste do MCA
res.mca_pref_2 <- MCA(db_bloco_2_pref, ncp = 5, graph = F)
```
...
```{r, fig.cap = "Figura n - Representação do quanto as duas principais dimensões explicam cada dividuo e variavel"}
fviz_mca_biplot(res.mca_pref_2,
  repel = TRUE, # Avoid text overlapping (slow if many point)
  ggtheme = theme_minimal()
)
```
...
```{r, fig.cap = "Figura n - Representação gráfica da porcentagem de variância explicada por cada dimensão"}
# Plot dos 10 maiores autovalores
fviz_screeplot(res.mca_pref_2, addlabels = TRUE, ylim = c(0, 10))
```
...
par(mfrow = c(2, 1))
# Contributions of rows to dimension 1
fviz_contrib(res.mca_pref_2, choice = "var", axes = 1, top = 15)
# Contributions of rows to dimension 2
fviz_contrib(res.mca_pref_2, choice = "var", axes = 2, top = 15)
```
...
# Plot de gráficos de barras para a quantidade de respostas por categoria por variável
par(mfrow = c(2, 4))
plot(db_bloco_2_posf[, 1],
  main = "Qtd Livros impressos para crianças",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_posf[, 2],
  main = "Qtd Livros digitais para crianças (e-
books no Kindle)",
  ylab = "Count",
  col = "steelblue",
...
plot(db_bloco_2_posf[, 3],
  main = "Qtd Jogos educativos de tabuleiro/cartas",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_posf[, 4],
  main = "Qtd Jogos educativos digitais",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_posf[, 5],
  main = "Qtd Livros impressos para adultos",
  ylab = "Count",
  col = "steelblue",
  las = 2
)
plot(db_bloco_2_posf[, 6],
  main = "Qtd Livros digitais ou assinaturas de jornais para
adultos",
  ylab = "Count",
  col = "steelblue",
...
```{r}
res.mca_posf_2 <- MCA(db_bloco_2_posf, ncp = 5, graph = F)
```
...
```{r, fig.cap = "Figura n - Representação do quanto as duas principais dimensões explicam cada dividuo e variavel"}
fviz_mca_biplot(res.mca_posf_2,
  repel = TRUE, # Avoid text overlapping (slow if many point)
  ggtheme = theme_minimal()
)
```
...
```{r, fig.cap = "Figura n - Representação gráfica da porcentagem de variância explicada por cada dimensão"}
# Plot dos 10 maiores autovalores
fviz_screeplot(res.mca_posf_2, addlabels = TRUE, ylim = c(0, 10))
```
...
par(mfrow = c(2, 1))
# Contributions of rows to dimension 1
fviz_contrib(res.mca_posf_2, choice = "var", axes = 1, top = 15)
# Contributions of rows to dimension 2
fviz_contrib(res.mca_posf_2, choice = "var", axes = 2, top = 15)
```
...
```{r, fig.cap = "Figura n - Quantidade de respostas por alternativa"}
# Plot de gráficos de barras para a quantidade de respostas por categoria por variável
par(mfrow = c(2, 4))
...
plot(db_bloco_3_pref[, 5],
  main = colnames(db_bloco_3_pref)[7],
  ylab = "Count",
  col = "steelblue",
  las = 2
)
...
Representação gráfica da porcentagem de variância explicada por cada dimensão.
```{r, fig.cap = "Figura n - Representação gráfica da porcentagem de variância explicada por cada dimensão"}
fviz_screeplot(res.mca_pref_3, addlabels = TRUE, ylim = c(0, 10))
```
Quanto cada uma das alternativas do questionários contribuem para as dimensções 1 e 2
...
```{r, fig.cap = "Figura n - Correlação entre variáveis e dimensões 1 e 2"}
...
```{r, fig.cap = "Figura n - Contribuição de cada alternativa para a composição das componentes 1 e 2"}
# Contributions of rows to dimension 1
fviz_contrib(res.mca_pref_3, choice = "var", axes = 1, top = 15)
# Contributions of rows to dimension 2
fviz_contrib(res.mca_pref_3, choice = "var", axes = 2, top = 15)
...
Assim com no biplot pré-pandemia, podemos observar que há uma grande quantidade de indivíduos que não são explicados pelas duas principais dimensões.
```{r, fig.cap = "Figura n - Representação do quanto as duas principais dimensões explicam cada dividuo e variavel"}
...
Pode-se observar que para as duas principais dimensões, a maioria das variáveis têm um correlação alta, destoando as variáveis **ea_indr_since** e **ea_read_since**.
```{r, fig.cap = "Figura n - Correlação entre variáveis e dimensões 1 e 2"}
