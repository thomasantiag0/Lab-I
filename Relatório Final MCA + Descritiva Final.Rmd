...
```{r}
for (i in 1:7) {
  plot(db_bloco_2_pref[, i],
    main = colnames(db_bloco_2_pref)[i],
...
```{r, include=FALSE}
res.mca_pref <- MCA(db_bloco_2_pref, ncp = 5, graph = F)
```
...
```{r, include=FALSE}
res.mca_posf <- MCA(db_bloco_2_posf, ncp = 5, graph = F)
```
...
t_pos <- "Pós Pandemia"
par(mfrow = c(1, 2))
corrplot(matcorr_pre_3, method = "circle", type = "upper", diag = FALSE, title = "Pré Pandemia", addCoef.col = "black", mar = c(0, 0, 2, 0))
