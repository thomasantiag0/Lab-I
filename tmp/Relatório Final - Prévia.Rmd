...
Pode-se observar que as variáveis que apresentam maior correlação são justamente aquelas variáveis pré e pós pandemais, tal como**hlr_books_b4** e **hlr_books_since** que indicam a quantidade de Livros impressos para crianças, **hlr_ad_books_b4** e **hlr_ad_books_since**, que indicam a quantidade Livros impressos para adultos. Outra correlação é entre **hlr_ad_books_b4** e **hlr_books_since**.
...
> Mapa de calor para a correlação entre as variáveis do bloco 2 pré-pandemia

```{r}
matcorr_pre_2 <- cor(db_bloco_2_pre)
corrplot(matcorr_pre_2, method="circle", type = "upper")
```

> Mapa de calor para a correlação entre as variáveis do bloco 2 pós-pandemia

```{r}
matcorr_pos_2 <- cor(db_bloco_2_pos)
corrplot(matcorr_pos_2, method="circle")
```
