...
### Matriz de correlação entre as variáveis do bloco 2
### Mapa de calor para a correlação entre as variáveis do bloco 3

Pode-se observar que as variáveis que apresentam maior correlação são justamente aquelas variáveis pré e pós pandemais, tal como 
**hlr_books_b4** e **hlr_books_since** que indicam a quantidade de Livros impressos para crianças, **hlr_ad_books_b4** e **hlr_ad_books_since**, que indicam a quantidade Livros impressos para adultos. Outra correlação é entre **hlr_ad_books_b4** e **hlr_books_since**, **hlr_books_since** e **hlr_dgames_since** e 
```{r}
matcorr_2 <- cor(db_bloco_2)
print(matcorr_2, digits = 2)
corrplot(matcorr_2, method="circle", type = 'upper')
```
...
```{r, include=FALSE}
res.mca_2f <- MCA(db_bloco_2f,ncp = 5, graph = TRUE ) 
```
...
fviz_mca_biplot(res.mca_2f, 
              ...
              ylim= c(-4,2))
```
A variável **hlr_ad_news_since** e **hlr_ad_news_b4** é a menos correlacionada com ambas dimensões.
As demais variáveis sao próximas nos dos primeiros componentes. 
...
As 5 primeiras dimensões juntas explicam 22% da variância dos dados.
E as 2 primeiras 11%.
Porcentagens de inércia explicadas por cada dimensão MCA

```{r}
fviz_screeplot(res.mca_2f, addlabels = TRUE, ylim = c(0, 10))
```
15 principais categorias de variáveis que contribuem para as dimensões:
...
Para o teste de esfericidade de Bartletti o p-valor é praticamente 0, a matrix de correlação não é diagonal, ou seja, as variáveis são correlacionadas. E pode ser feita a análise.

Todas as variaa1áveis concordam para a primeira componente. 
Para a segunda componente **books_b4** e **ad_books_b4** e **games_b4** tem  efeito negativo
```{r}
